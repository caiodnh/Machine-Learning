...
def epsilon_greedy(state_vector, epsilon):
    ...
    if draw <= epsilon:
        ...
        actions, objects = model.forward(state_vector)
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    q_cur = 1/2 * (q_values_action_cur[action_index] + q_values_object_cur[object_index])

    sample = reward + GAMMA * maxq_next

    loss = 1/2 * (sample - q_cur)**2
