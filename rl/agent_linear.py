...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    if draw <= epsilon:
        ...
        command_values = theta @ state_vector
        return np.unravel_index(np.argmax(command_values), (NUM_ACTIONS, NUM_OBJECTS)) 
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    if terminal:
        ...
        v_next_state = np.amax(theta @ next_state_vector)
    sample = reward + GAMMA * v_next_state
    
    command_index = tuple2index(action_index, object_index)
    current_q_value = np.dot(theta[command_index], current_state_vector)
    
    theta[command_index] = theta[command_index] + ALPHA * (sample - current_q_value) * current_state_vector
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        # Translate new state to vectors
...
def run():
    ...
    print(single_run_epoch_rewards_test.pop())
